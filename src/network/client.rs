...
use tokio::sync::mpsc;
use log::{debug, warn, error}; // Added logging macros
            ...
            match command {
                OutboundCommand::SendRequestVote { target_node_id, args, result_channel } => {
                    let Some(mut client) = self.peers.get(&target_node_id).cloned() else {
                        error!("Cannot send RequestVote: Unknown target node ID {}", target_node_id);
                    ...
                    debug!("Sending RequestVote to Node {} for term {}", target_node_id, args.term);
                        ...
                        loop {
                            match client.request_vote(proto_args.clone()).await {
                                Ok(response) => {
                                    ...
                                    debug!("Received RequestVote reply from Node {}: vote_granted={}", target_node_id, res.vote_granted);
                                ...
                                Err(status) => {
                                    attempts += 1;
                                    warn!("RequestVote to Node {} failed (attempt {}): {}", target_node_id, attempts, status.message());
                                    
                                    if attempts >= MAX_MICRO_RETRIES || !is_transient_error(&status) {
                                        error!("RequestVote to Node {} aborted after {} attempts: {}", target_node_id, attempts, status.message());
                ...
                OutboundCommand::SendAppendEntries { target_node_id, args, result_channel } => {
                    let Some(mut client) = self.peers.get(&target_node_id).cloned() else {
                        error!("Cannot send AppendEntries: Unknown target node ID {}", target_node_id);
                        let _ = result_channel.send(Err("Unknown target node ID".to_string()));
                        continue;
                    };

                    let proto_entries: Vec<_> = args.entries
                        .into_iter()
                    ...
                    let entry_count = proto_entries.len();
                    ...
                    debug!("Sending AppendEntries to Node {} (term: {}, entries: {})", target_node_id, args.term, entry_count);
                    ...
                    tokio::spawn(async move {
                        ...
                        loop {
                            match client.append_entries(proto_args.clone()).await {
                                Ok(response) => {
                                    let res = response.into_inner();
                                    debug!("Received AppendEntries reply from Node {}: success={}", target_node_id, res.success);
                                ...
                                Err(status) => {
                                    attempts += 1;
                                    warn!("AppendEntries to Node {} failed (attempt {}): {}", target_node_id, attempts, status.message());
                                    
                                    if attempts >= MAX_MICRO_RETRIES || !is_transient_error(&status) {
                                        error!("AppendEntries to Node {} aborted after {} attempts: {}", target_node_id, attempts, status.message());
                                        let _ = result_channel.send(Err(format!(
                                            "gRPC Error after {} attempts: {}", 
                                            attempts, status.message()
                                        )));
