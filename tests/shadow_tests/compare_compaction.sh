...
#
# Runs the log_compaction shadow test twice — once with compaction enabled
# (default RAFT_COMPACTION_THRESHOLD=50) and once with compaction effectively
# disabled (RAFT_COMPACTION_THRESHOLD set very high) — and compares the
# resulting on-disk WAL sizes for each node via wal_dump's
# `on_disk_size_bytes` field.
#
